...
/** Writes ISO-8859-1 / Windows-1252 bytes (PUZ string fields). */
function insertString(bytes: Uint8Array, str: string, pos: number) {
    for (const ch of str) {
        const c = ch.codePointAt(0)!;
        if (c <= 0xff) {
            bytes[pos++] = c;
        } else if (c in WINDOWS_1252_UNICODE_TO_BYTE) {
            bytes[pos++] = WINDOWS_1252_UNICODE_TO_BYTE[c];
        } else {
            throw new Error(`PUZ string must be ISO-8859-1 or Windows-1252; cannot encode U+${c.toString(16)}`);
        }
    }
}
